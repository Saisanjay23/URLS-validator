"""
FastAPI application — Stateless URL validation microservice (API only).

Headless build for server-to-server integration: Java sends URLs → Python
returns results → done. No database, no state, no UI. Same validation logic
as the full build; only the frontend mount is removed.
"""
...
app = FastAPI(
    title="Social URL Status Checker (API)",
    description="Enterprise-grade bulk URL validation engine. Check whether social media URLs are active or taken down.",
    version="5.0.0",
)

# Java calls are server-to-server (CORS not applicable). Browser origins, if
# ever needed, come from URLCHECK_ALLOWED_ORIGINS.
app.add_middleware(
    CORSMiddleware,
...
@app.get("/")
async def root():
    """Service identity for load balancers / smoke tests."""
    return {
        "service": "social-url-status-checker",
        "mode": "api-only",
        "version": "5.0.0",
        "endpoints": ["/api/health", "/api/check/json", "/api/check", "/api/metrics", "/api/export", "/api/cookies"],
    }
...
async def check_urls_json(request: URLCheckRequest):
    ...
    NOTE: invalid URLs are dropped and duplicates are collapsed before checking,
    so match results to inputs by URL, not by index/count.
    """
...
async def check_urls_stream(request: URLCheckRequest):
    """Stream URL check results as Server-Sent Events."""
    if len(request.urls) > MAX_URLS_PER_REQUEST:
        raise HTTPException(status_code=400, detail=f"Maximum {MAX_URLS_PER_REQUEST} URLs per request.")
